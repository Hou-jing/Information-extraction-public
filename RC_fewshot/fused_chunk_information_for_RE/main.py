...
def evaluate(test_batch, args):
    ...
    with torch.no_grad():
        for data in test_batch:
            ...
            sup_loss,loss, logits = model(text, label)
            test_loss += loss.item()
            key.extend(label[::2].cpu())
            predict.extend(torch.argmax(logits,1).cpu())
    key=key
    predict=predict
...
if __name__=='__main__':
    ...
    parser.add_argument('--embed_mode',default='bert_cased',choices=['bert_cased','albert'])
    ...
    for epoch in range(args.epoch):
        ...
        for data in tqdm(train_batch):
            ...
            text = data[0]
            label = torch.tensor(data[1]).to(device)
            sup_loss,kl_loss, logits=model(text,label)

            # kl_loss.backward(retain_graph=True)
            sup_loss.backward(retain_graph=True)
            train_loss += sup_loss.item()+kl_loss.item()
            optimizer.step()
            # scheduler.step()
            if steps % args.steps == 0:
                logger.info("Epoch: {}, step: {} / {}, loss = {:.4f},sup_loss = {:.4f},kl_loss={},learning_rate={}".format
                            (epoch, steps, len(train_batch), train_loss / steps,sup_loss,kl_loss, optimizer.param_groups[0]['lr']))
