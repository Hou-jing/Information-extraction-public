...
def json_loads(path, name,rel2idx):
    ...
    with open(file,'r',encoding='utf_8') as f:
        ...
        for line in fp:
            ...
            data.append([' '.join(a['token']),rel2idx[a['relation']]])
    return data
...
def dataloader(args):
    path = "E:/PythonProject/pythonProject_22.10.24_few_shot/小样本想法/dataset/" + args.data
    with open("E:/PythonProject/pythonProject_22.10.24_few_shot/小样本想法/dataset/" + args.data + "/rel2id.json", "r") as f:
        rel2idx = json.load(f)
    # if args.data == "retacred":
    train_data = json_loads(path + '/' + 'k-shot/' + args.shot + '/', "train.txt",rel2idx)
    test_data = json_loads(path, "test.txt",rel2idx)
    dev_data = json_loads(path, 'val.txt',rel2idx)
